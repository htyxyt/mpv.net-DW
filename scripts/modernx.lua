...
local user_opts = {
    ...
    bottomhoverheight = 200,        -- the area height for hovering at the bottom
    ...
    layoutoption = "original",	    -- switching the original/reduced layouts
...
layouts["reduced"] = function ()
    ...
    -- Title
    geo = {x = 25, y = refY - 97 + (((state.localDescription ~= nil or state.isWebVideo) and user_opts.showdescription) and -20 or 0), an = 1, w = osc_geo.w - 50, h = 35}
    lo = add_layout("title")
    lo.geometry = geo
    lo.style = string.format("%s{\\clip(0,%f,%f,%f)}", osc_styles.Title,
                             geo.y - geo.h, geo.x + geo.w, geo.y + geo.h)
    ...
    -- Description
    if (state.localDescription ~= nil or state.isWebVideo) and user_opts.showdescription then
        geo = {x = 25, y = refY - 97, an = 1, w = osc_geo.w - 80, h = 19}
        lo = add_layout("description")
        lo.geometry = geo
        lo.style = osc_styles.Description
        lo.alpha[3] = 0
...
function osc_init()
    ...
    ne.eventresponder["mbtn_left_up"] = function ()
        if user_opts.layoutoption == "original" then
    ...
    -- load layout
    layouts[user_opts.layoutoption]()
    ...
    elseif source == 'mouse_move' then
        ...
        if (user_opts.minmousemove == 0) or (not ((state.last_mouseX == nil) or (state.last_mouseY == nil)) and ((math.abs(mouseX - state.last_mouseX) >= user_opts.minmousemove) or (math.abs(mouseY - state.last_mouseY) >= user_opts.minmousemove))) then
                if user_opts.bottomhover then -- if enabled, only show osc if mouse is hovering at the bottom of the screen (where the UI elements are)
                    if (mouseY > osc_param.playresy - user_opts.bottomhoverheight) or ((not state.border or state.fullscreen) and mouseY < 40) then -- account for scaling options
                        show_osc()
                    else
                        hide_osc()
                    end
